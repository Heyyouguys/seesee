...
const AIRecommendConfig = ({ config, refreshConfig }: AIRecommendConfigProps) => {
  ...
  const fetchModels = async () => {
    ...
    try {
      // 构造models API地址
      let modelsUrl = aiSettings.apiUrl.trim();
      ...
      if (data.data && Array.isArray(data.data)) {
        models = data.data
          ...
          .filter((id: string) => id && !id.includes('embedding') && !id.includes('whisper') && !id.includes('tts'))
          .sort();
      ...
      if (models.length === 0) {
        showMessage('error', '未能获取到可用模型，请手动输入');
      } else {
        setFetchedModels(models);
        showMessage('success', `成功获取 ${models.length} 个模型`);
      }
    } catch (err) {
      ...
      <div className='bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700 shadow-sm'>
        ...
        <div className='mb-6'>
          <label className='flex items-center cursor-pointer'>
            ...
            />
            <div className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${aiSettings.enabled
              ? 'bg-blue-600'
                ...
                {showCustomModelInput ? (
                  <div className='space-y-2'>
                    <input
                      ...
                      className='w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500'
                      placeholder='请输入自定义模型名称，如：gpt-5'
                    />
                    <button
                      type='button'
                      onClick={() => {
                        setShowCustomModelInput(false);
                        // 如果自定义值不为空,保持当前值;否则恢复到第一个预设模型
                        if (!customModelValue.trim()) {
                          const firstModel = fetchedModels.length > 0 ? fetchedModels[0] : MODEL_EXAMPLES[0].split(' (')[0];
                          setAiSettings(prev => ({ ...prev, model: firstModel }));
                        }
                      }}
                  ...
                  <div className='space-y-2'>
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        if (value === '__custom__') {
                          ...
                          setCustomModelValue(aiSettings.model);
                        } else {
                    ...
                    <p className='text-xs text-gray-500 dark:text-gray-400'>
                      当前选择: <code className='bg-gray-100 dark:bg-gray-800 px-1 rounded'>{aiSettings.model}</code>
                    </p>
                  </div>
                )}

                <p className='text-xs text-orange-600 dark:text-orange-400'>
                  ⚠️ 建议使用支持联网搜索的模型以获得更好的推荐效果
                </p>
              </div>
            </div>
